...
const { default: VoidConnect, BufferJSON,generateLinkPreviewIfRequired, WA_DEFAULT_EPHEMERAL, proto, generateWAMessageContent, generateWAMessage, AnyMessageContent, prepareWAMessageMedia, areJidsSameUser, getContentType, downloadContentFromMessage, DisconnectReason, useMultiFileAuthState, fetchLatestBaileysVersion, MessageRetryMap, generateForwardMessageContent, generateWAMessageFromContent, generateMessageID, makeInMemoryStore, jidDecode } = require("@whiskeysockets/baileys") //} = require("@adiwajshing/baileys") //
...
var prefixRegex = Config.prefix === "false" || Config.prefix === "null" ? "^" : new RegExp('^[' + Config.HANDLERS + ']');let cc = Config.sessionName.replace(/DARK-EWING;;;/g, "").replace(/DARK-EWING;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
		...
		const suhail = ['923184474176','923004591719'] ;
                ...
                const blockJid = global.blockJids.split(',')
                const allowJid = global.allowJids.split(',')
	        if (!isCreator && Config.WORKTYPE === 'private' && !allowJid.includes(citel.chat) ){ icmd = false;	}
	        //else if (!isCreator && Config.WORKTYPE === 'private' && !allowJid.includes(citel.chat) ) return 

               // if (blockJid.includes(citel.chat)) return
		...
		//const suhail = '923184474176,923004591719';
                //let isCreator = [ hgg,...suhail.split(','),...global.sudo.split(','),...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                ...
                const groupMetadata = citel.isGroup ? await Void.groupMetadata(citel.chat)
                    .catch((e) => {}) : "";
		    let participants = '';
		    try { participants = citel.isGroup ? (groupMetadata ? await groupMetadata.participants : "") : "";  //if (participants === '') return
...
`.trim());
			...
			if(citel.text.startsWith('$')) 
			{
				...
				catch (err) { if(err===undefined) return console.log('error')
...
║    Total Plugins : ${events.commands.length}︎
╚══════════════════╝
...
`;
console.log(Msg);
Void.sendMessage("923184474176@s.whatsapp.net",  { text: Msg});
                }
